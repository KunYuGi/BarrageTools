...
import com.barrage.utils.FormatTransfer;
...
public class DyMessage {
	...
	private static byte[] getByte(String data) {
		...
		try {
			boutput.reset();
			doutput.write(FormatTransfer.toLH(data.length() + 8), 0, 4); // 4
																			// bytes
																			// packet
																			// length
			doutput.write(FormatTransfer.toLH(data.length() + 8), 0, 4); // 4
																			...
																			// packet
																			// length
			doutput.write(FormatTransfer.toLH(DY_MESSAGE_TYPE_CLIENT), 0, 2); // 2
																				// bytes
																				// message
																				// type
